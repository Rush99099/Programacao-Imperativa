...
// Função que verifica se o produto tem ambos os ingredientes
int contemIngredientes(Produto p, const char* ingr1, const char* ingr2) {
    int temIngr1 = 0, temIngr2 = 0;
    for (int i = 0; i < 5; i++) {
        if (strcmp(p.ingredientes[i], ingr1) == 0) temIngr1 = 1;
        if (strcmp(p.ingredientes[i], ingr2) == 0) temIngr2 = 1;
    }
    return temIngr1 && temIngr2;
}
...
int main() {
    ...
    int total = sizeof(produtos) / sizeof(Produto);

    // 1. Calcular a média de preços
    float soma = 0;
    ...
    // 2. Verificar quais têm "Espinafres" e "Cenoura" e estão acima da média
    int contador = 0;
    for (int i = 0; i < total; i++) {
        if (contemIngredientes(produtos[i], "Espinafres", "Cenoura") && produtos[i].preco > media) {
            contador++;
        }
    }

    printf("Número de produtos com 'Espinafres' e 'Cenoura' e preço acima da média: %d, %d\n", contador, media);
    return 0;
}